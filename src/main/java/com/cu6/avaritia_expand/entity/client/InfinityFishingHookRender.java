...
public class InfinityFishingHookRender extends EntityRenderer <InfinityFishingHook> {
    private static final ResourceLocation TEXTURE_LOCATION = new ResourceLocation(AvaritiaExpand.MOD_ID, "textures/entity/infinity_fishing_hook.png");
    private static final RenderType RENDER_TYPE;
    private static final double VIEW_BOBBING_SCALE = (double)960.0F;

    public InfinityFishingHookRender(EntityRendererProvider.Context pContext) {
        super(pContext);
    }

    public void render(InfinityFishingHook pEntity, float pEntityYaw, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        if (player != null) {
            ...
            if ((this.entityRenderDispatcher.options == null || this.entityRenderDispatcher.options.getCameraType().isFirstPerson()) && player == Minecraft.getInstance().player) {
                double d7 = (double)960.0F / (double)(Integer)this.entityRenderDispatcher.options.fov().get();
            ...
            double d10 = Mth.lerp((double)pPartialTicks, pEntity.yo, pEntity.getY()) + (double)0.25F;
    ...
    private static void stringVertex(float pX, float pY, float pZ, VertexConsumer pConsumer, PoseStack.Pose pPose, float p_174124_, float p_174125_) {
        ...
        f4 /= f6;
        f5 /= f6;
        pConsumer.vertex(pPose.pose(), f, f1, f2).color(56, 127, 123, 255).normal(pPose.normal(), f3, f4, f5).endVertex();
    }

    public ResourceLocation getTextureLocation(InfinityFishingHook pEntity) {
