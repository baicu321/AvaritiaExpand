...
public class CrystalTNTEntity extends Entity implements TraceableEntity {
    ...
    public CrystalTNTEntity(EntityType<CrystalTNTEntity> type, Level level) {
        super(type, level);
//半径等于什么不重要,在CrystalTNT类中设置
        this.explosionRadius = 5;
    }

    public CrystalTNTEntity(Level pLevel, double pX, double pY, double pZ, @Nullable LivingEntity pOwner) {
        this(ModEntities.CRYSTAL_TNT_ENTITY.get(), pLevel);
            ...
            for (int x = -(int) radius; x <= radius; x++) {
                for (int z = -(int) radius; z <= radius; z++) {
                    ...
                    if ((x*x + z*z) <= radius * radius) {
                        ...
                        for (int yOffset = -20; yOffset <= 10; yOffset++) {
    ...
    static {
        DATA_FUSE_ID = SynchedEntityData.defineId(CrystalTNTEntity.class, EntityDataSerializers.INT);
    }
}
